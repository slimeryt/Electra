...
export function createMessage(channelId: string, authorId: string, content: string | undefined, fileIds: string[]) {
  return db.transaction(() => {
    const msg = db.prepare(
      'INSERT INTO messages (channel_id, author_id, content, type) VALUES (?, ?, ?, ?) RETURNING *'
    ).get(channelId, authorId, content || null, fileIds.length > 0 ? 'file' : 'text') as Message;
