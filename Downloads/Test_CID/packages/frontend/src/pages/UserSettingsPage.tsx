...
function parseAccentColor(v: string | null | undefined) {
  if (!v) return { mode: 'solid' as const, c1: '#5865f2', c2: '#4752c4', angle: 135 };
  if (v.startsWith('linear-gradient')) {
    const m = v.match(/linear-gradient\((\d+)deg,\s*(#[0-9a-fA-F]{3,8}),\s*(#[0-9a-fA-F]{3,8})\)/);
    if (m) return { mode: 'gradient' as const, angle: parseInt(m[1]), c1: m[2], c2: m[3] };
  }
  return { mode: 'solid' as const, c1: v, c2: '#4752c4', angle: 135 };
}
...
const NAV_ITEMS: { tab: SettingsTab; label: string; icon: React.ReactNode }[] = [
  { tab: 'profile',    label: 'Profile',    icon: <User size={15} /> },
  { tab: 'appearance', label: 'Appearance', icon: <Palette size={15} /> },
  { tab: 'account',    label: 'Account',    icon: <KeyRound size={15} /> },
];
...
export default function UserSettingsPage() {
  ...
  const handleAvatarUpload = async (file: File) => {
    ...
    try {
      ...
      setUser(data.user || data);
    } catch (e: any) { setError(e.response?.data?.error || 'Failed to upload avatar'); }
  ...
  const handleBannerUpload = async (file: File) => {
    ...
    try {
      ...
      setUser(data.user || data);
    } catch (e: any) { setError(e.response?.data?.error || 'Failed to upload banner'); }
  ...
  const handleRemoveBanner = async () => {
    try {
      ...
      setUser(data.user || data);
    } catch (e: any) { setError(e.response?.data?.error || 'Failed to remove banner'); }
  };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden', height: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', padding: '0 8px', marginBottom: 6 }}>
          User Settings
        </div>
        {NAV_ITEMS.map(item => (
          <button
            ...
            style={{
              ...
              fontSize: 13, fontWeight: activeTab === item.tab ? 600 : 400,
              background: activeTab === item.tab ? 'rgba(255,255,255,0.07)' : 'transparent',
              color: activeTab === item.tab ? 'var(--text-primary)' : 'var(--text-secondary)',
              textAlign: 'left', transition: 'all 120ms', marginBottom: 2,
            }}
            onMouseEnter={e => { if (activeTab !== item.tab) e.currentTarget.style.background = 'rgba(255,255,255,0.04)'; }}
        ...
        {/* Divider + sign out at bottom */}
        <div style={{ marginTop: 'auto', paddingTop: 16, borderTop: '1px solid var(--border)', marginLeft: -8, marginRight: -8, paddingLeft: 8, paddingRight: 8 }}>
          <button
            ...
            style={{
              ...
              padding: '8px 10px', borderRadius: 'var(--radius-md)',
              border: 'none', cursor: 'pointer', fontFamily: 'inherit',
              fontSize: 13, fontWeight: 400, width: '100%', textAlign: 'left',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <div style={{ position: 'relative', flexShrink: 0 }}>
                      ...
                      <button
                        ...
                        style={{ position: 'absolute', inset: 0, borderRadius: '50%', background: 'rgba(0,0,0,0.6)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity 150ms', color: '#fff' }}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6 }}>
                    {FONTS.map(f => (
                      <button key={f.value} onClick={() => setSelectedFont(f.value)} style={{
                        ...
                        borderRadius: 'var(--radius-md)', background: selectedFont === f.value ? 'rgba(88,101,242,0.12)' : 'var(--bg-overlay)',
                ...
                <Card>
                  ...
                  <textarea
                    ...
                    placeholder="Tell others a bit about yourself…" maxLength={300} rows={3}
                    style={{ width: '100%', boxSizing: 'border-box', padding: '8px 12px', background: 'var(--bg-overlay)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', color: 'var(--text-primary)', fontSize: 13, fontFamily: 'inherit', resize: 'vertical', outline: 'none', lineHeight: 1.5, transition: 'border-color 150ms' }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 14 }}>
                    ...
                    <button onClick={() => setShowBadges(v => !v)} style={{ width: 40, height: 22, borderRadius: 11, border: 'none', background: showBadges ? 'var(--accent)' : 'var(--bg-overlay)', cursor: 'pointer', position: 'relative', flexShrink: 0, transition: 'background 200ms', outline: '1px solid var(--border)' }}>
                      <div style={{ position: 'absolute', top: 3, left: showBadges ? 21 : 3, width: 16, height: 16, borderRadius: '50%', background: '#fff', transition: 'left 200ms' }} />
                    </button>
                  </div>
                </Card>
                ...
                {error && (
                  <div style={{ padding: '10px 14px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 'var(--radius-md)', color: 'var(--danger)', fontSize: 13 }}>
                    {error}
                  </div>
                )}
          ...
          {activeTab === 'appearance' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 24, alignItems: 'start' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    {THEMES.map(t => (
                      <button key={t.value} onClick={() => handleThemeChange(t.value)} style={{
                        ...
                        <div style={{ display: 'flex', gap: 2, flexShrink: 0 }}>
                          {t.preview.map((c, i) => (
                            <div key={i} style={{ width: i === 2 ? 10 : 14, height: 24, background: c, borderRadius: i === 0 ? '3px 0 0 3px' : i === 1 ? '0' : '0 3px 3px 0' }} />
                          ))}
                        </div>
                        <span style={{ fontSize: 13, fontWeight: currentTheme === t.value ? 600 : 400, color: currentTheme === t.value ? 'var(--accent)' : 'var(--text-primary)' }}>{t.label}</span>
                        {currentTheme === t.value && <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--accent)', fontWeight: 600 }}>✓</span>}
                      </button>
                    ))}
                  </div>
                </Card>
                ...
                {/* Profile Banner */}
                <Card>
                  <SectionLabel>Profile Banner</SectionLabel>
                      ...
                      {colorMode === 'solid' ? (
                        <>
                          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginBottom: 10 }}>
                            {SOLID_PRESETS.map(c => (
                              <button key={c} onClick={() => setSolidColor(c)} style={{ width: 30, height: 30, borderRadius: '50%', background: c, border: 'none', outline: solidColor === c ? '3px solid #fff' : 'none', outlineOffset: 2, cursor: 'pointer', padding: 0, flexShrink: 0, boxShadow: '0 0 0 1px rgba(255,255,255,0.08)', transition: 'transform 100ms' }}
                        ...
                        <>
                          <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginBottom: 14 }}>
                            {GRADIENT_PRESETS.map(p => (
                              <button key={p.label} onClick={() => { setGradColor1(p.c1); setGradColor2(p.c2); setGradAngle(p.angle); }} title={p.label} style={{ width: 30, height: 30, borderRadius: '50%', border: 'none', background: buildGradient(p.c1, p.c2, p.angle), cursor: 'pointer', padding: 0, flexShrink: 0, outline: (gradColor1 === p.c1 && gradColor2 === p.c2) ? '3px solid #fff' : 'none', outlineOffset: 2, boxShadow: '0 0 0 1px rgba(255,255,255,0.08)', transition: 'transform 100ms' }}
                                onMouseEnter={e => { e.currentTarget.style.transform = 'scale(1.15)'; }}
                                onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)'; }}
                              />
                            ))}
                          ...
                          <div style={{ marginBottom: 12 }}>
                            ...
                            <div style={{ display: 'flex', gap: 6 }}>
                              {[45, 90, 135, 180].map(a => (
                                <button key={a} onClick={() => setGradAngle(a)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', border: `1px solid ${gradAngle === a ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 'var(--radius-sm)', background: gradAngle === a ? 'rgba(88,101,242,0.15)' : 'var(--bg-overlay)', color: gradAngle === a ? 'var(--accent)' : 'var(--text-secondary)', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>
                    ...
                    <div>
                      {user?.banner_url ? (
                        <div style={{ position: 'relative', borderRadius: 'var(--radius-md)', overflow: 'hidden', marginBottom: 10 }}>
                          ...
                          <button onClick={handleRemoveBanner} style={{ position: 'absolute', top: 6, right: 6, background: 'rgba(0,0,0,0.6)', border: 'none', borderRadius: '50%', width: 22, height: 22, cursor: 'pointer', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
                            <X size={11} />
                          </button>
                        </div>
                      ) : (
                      ...
                      <button onClick={() => bannerInputRef.current?.click()} disabled={isUploadingBanner} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 14px', background: 'var(--bg-overlay)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', color: 'var(--text-primary)', cursor: 'pointer', fontSize: 13, fontFamily: 'inherit' }}>
                        {isUploadingBanner ? <div style={{ width: 14, height: 14, border: '2px solid var(--text-muted)', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} /> : <ImagePlus size={14} />}
            ...
            <div style={{ maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <Card>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {[
                    ...
                    <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'var(--bg-overlay)', borderRadius: 'var(--radius-md)' }}>
                      <span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 500 }}>{row.label}</span>
                      <span style={{ fontSize: row.label === 'User ID' ? 11 : 13, color: 'var(--text-primary)', fontFamily: row.label === 'User ID' ? 'monospace' : 'inherit' } as React.CSSProperties}>{row.value}</span>
                    </div>
                  ))}
                </div>
              </Card>
              ...
              <Card>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                  <SectionLabel>Saved Accounts ({savedAccounts.length}/5)</SectionLabel>
                  <button
                    onClick={() => {
                      const accounts = getSavedAccounts();
                      const alreadySaved = accounts.some(a => a.userId === user?.id);
                      if (!alreadySaved && accounts.length >= 5) return;
                      upsertSavedAccount(user!);
                      setSavedAccounts(getSavedAccounts());
                    }}
                    disabled={savedAccounts.length >= 5 && !savedAccounts.some(a => a.userId === user?.id)}
                    style={{ fontSize: 11, color: (savedAccounts.length >= 5 && !savedAccounts.some(a => a.userId === user?.id)) ? 'var(--text-muted)' : 'var(--accent)', background: 'none', border: 'none', cursor: (savedAccounts.length >= 5 && !savedAccounts.some(a => a.userId === user?.id)) ? 'default' : 'pointer', fontFamily: 'inherit', padding: '2px 6px', borderRadius: 'var(--radius-sm)' }}
                  >
                    {savedAccounts.length >= 5 && !savedAccounts.some(a => a.userId === user?.id) ? '5/5 limit reached' : 'Save current'}
                  </button>
                </div>

                {savedAccounts.length === 0 ? (
                  <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13, background: 'var(--bg-overlay)', borderRadius: 'var(--radius-md)' }}>
                    No saved accounts yet. Click "Save current" to add this account.
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {savedAccounts.map(acc => {
                      ...
                      return (
                        <div key={acc.userId} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: isCurrent ? 'rgba(88,101,242,0.08)' : 'var(--bg-overlay)', borderRadius: 'var(--radius-md)', border: `1px solid ${isCurrent ? 'rgba(88,101,242,0.3)' : 'var(--border)'}` }}>
                          {/* Avatar */}
                          <div style={{ width: 36, height: 36, borderRadius: '50%', flexShrink: 0, overflow: 'hidden', background: '#5865f2', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 15 }}>
                            {acc.avatarUrl
                              ? <img src={acc.avatarUrl} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />
                              : (acc.displayName || acc.username).charAt(0).toUpperCase()
                            }
                          </div>
                          {/* Info */}
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{acc.displayName}</div>
                            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>@{acc.username}</div>
                          </div>
                          {isCurrent && <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--accent)', background: 'rgba(88,101,242,0.15)', padding: '2px 7px', borderRadius: 999 }}>Active</span>}
                          {/* Actions */}
                          {!isCurrent && (
                            <button
                              onClick={() => switchToAccount(acc)}
                              title="Switch to this account"
                              style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', cursor: 'pointer', color: 'var(--text-secondary)', padding: '4px 8px', fontSize: 11, fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 4 }}
                            >
                              <RefreshCw size={11} /> Switch
                            </button>
                          )}
                          <button
                            onClick={() => { removeSavedAccount(acc.userId); setSavedAccounts(getSavedAccounts()); }}
                            title="Remove from saved"
                            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 4, borderRadius: 'var(--radius-sm)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              ...
              <Card>
                ...
                <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 14 }}>
                  Signing out will disconnect you from Electra on this device.
                </div>
